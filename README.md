...
> backend SaaS (NestJS + Prisma), et les artefacts produit (mockups,
> spécifications, journal d'avancement).
...
| 5 | Frontend chef d'atelier | ⬜ à faire (mockups HTML disponibles) |
...
│   └── package.json
│
└── mockups/                           # Mockups UX HTML statique (jetable)
...
## Démarrage rapide (4 services en local)
...
cd backend && cp .env.example .env && npm run prisma:migrate && npm run start:dev
...
**Mockups frontend** : `cd mockups && python3 -m http.server 8080`
puis [http://localhost:8080/index.html](http://localhost:8080/index.html).
...
## Architecture (Phase 4 V1, sans messaging)
...
Postgres = multi-workers safe sans Redis.

Pour le pre-flight rapide (~1 s), HTTP synchrone direct vers FastAPI.

Détails dans [`phase-4-report.md`](./phase-4-report.md).
...
| À quoi ressemble l'UX cible ? | [`mockups/README.md`](./mockups/README.md) (8 écrans HTML) |
...
| Lignes de code applicatif | ~12k Python + ~3k TypeScript |
| Phases complètes | **5/9** (0, 1, 2, 3, 4) |
| Mockups HTML disponibles | **8 écrans** (5 quotidien + 3 onboarding) |
| Tables Postgres | **15** (+ migrations Prisma) |
| Endpoints REST exposés | **~30** (workshops, machines, clients, orders, versions, solve-jobs, preflight-sessions, health) |
