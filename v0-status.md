...
| 3 | Agents LLM (tool use natif Claude API) | 🔵 en test | 2026-04-30 | — |
...
| 3.4 | Agent extraction — questionnaire arborescent → spec structurée | 🔵 en test | 2026-04-30 | — | 10 cas types, validation Pydantic stricte | Scaffolding livré : `QuestionnaireAgent` + `WorkshopSpec` Pydantic strict + prompt v1. **Trial 1/1 OK** (PME aero EN9100, 13 machines / 17 op estimés cohérents, mappings enums respectés, notes explicatives détaillées). À reproduire sur 4-5 profils variés (auto, médical, hors aero) avant ✅. Voir `experiments/agents-trial/results.md`. |
...
| 3.7 | Agent explication — placement OF + INFEASIBLE NL | 🔵 en test | 2026-04-30 | — | Évaluation manuelle sur 30 cas | Scaffolding livré : `ExplanationAgent` + `ExplanationOutput` (summary + reasons ≤5 + actions ≤3 + kind) + prompt v1. **Trial 1 — `placement` ÉCHEC** (Claude a omis `kind`) → prompt v1 durci (règle 5 renforcée, mention obligatoire en tête de Tâche). **Trial 2 — `infeasibility` OK** (3 reasons + 3 actions concrètes, 0 jargon solveur). À re-tester `placement` avec le prompt durci. |
| 3.8 | Modifications conversationnelles ("priorité 1 sur Safran") | 🔵 en test | 2026-04-30 | — | 10 modifications types testées avec validation humaine | Scaffolding livré : `ConversationalEditAgent` + `EditPlan` (actions auditables + needs_clarification path) + prompt v1. **Trial 1/1 OK** sur cas piège conditionnel (« si TOUR-01 trop chargé ») : Claude a produit 2 actions distinctes (priorité + reassign) en interprétant la condition floue (87 % vs 42 %). **Comportement pro-actif à observer** : reproduire sur 4-5 demandes incluant un cas "condition non tranchable". |
