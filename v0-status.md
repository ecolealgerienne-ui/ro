...
| 3.4 | Agent extraction — questionnaire arborescent → spec structurée | 🔵 en test | 2026-04-30 | — | 10 cas types, validation Pydantic stricte | Scaffolding livré : `QuestionnaireAgent` + `WorkshopSpec` Pydantic strict + prompt v1. Tests avec FakeLLMProvider OK (3 tests). **Validation empirique avec design partners reste à faire** sur 10 cas types. |
...
| 3.7 | Agent explication — placement OF + INFEASIBLE NL | 🔵 en test | 2026-04-30 | — | Évaluation manuelle sur 30 cas | Scaffolding livré : `ExplanationAgent` + `ExplanationOutput` (summary + reasons ≤5 + actions ≤3 + kind) + prompt v1. 4 tests. **Validation manuelle reste à faire** sur 30 cas types. |
| 3.8 | Modifications conversationnelles ("priorité 1 sur Safran") | 🔵 en test | 2026-04-30 | — | 10 modifications types testées avec validation humaine | Scaffolding livré : `ConversationalEditAgent` + `EditPlan` (actions auditables + needs_clarification path) + prompt v1. 7 catégories d'actions canoniques. Validator garantit cohérence clarification ↔ actions. 4 tests. **Validation manuelle reste à faire** sur 10 modifications types. |
